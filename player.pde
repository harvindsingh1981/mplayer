...
Process ffmpeg_c;
Process ffmpeg_r;
float default_text_size=12;
boolean is_init_done=false;
int loop_count=0;
void setup()
{
size(800,600,P3D);
...
//initialize();
println("omx running = "+check_if_omx_running());
img_booting=loadImage("/home/pi/Downloads/play_pause_icons/booting.png");
}
...
void draw()
{
...
else
  {
    ...
    println(width);
    println(height);
    image(img_booting,0,0,width,height);
    loop_count++;
    if(loop_count>5)
    {
    initialize();
