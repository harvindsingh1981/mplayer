...
/********************MQTT Settings********************************************************************/
#define MQTT_BROKER_IP                              "broker.mqtt-dashboard.com"//"192.168.43.225"
...
#define TOTAL_PUBLISH_TOPIC   5
int mqtt_topic_publish_status[TOTAL_PUBLISH_TOPIC][2];//To maintain status of topics which are published with QOS > 0(because publish messages with QOS less then 0 are not acknowledged by broker)
                                                      //Format [Topic(predefined)][[0]Subscriber ID [1]Status]
                                                      //Topic 0:Info
#define PUB_TOPIC_INFO_INDEX    0
#define PUB_TOPIC_STATUS_INDEX  1                                                                                                                    
/*****************************************************************************************************/
/***************Async Ping Library********************************************************************/
...
#define RED_LED     14 //12
#define GREEN_LED   12 //2
#define BLUE_LED    13 //4
#define LED         14 //12
bool led_status=0;
int led_intensity=0;
...
/*********Encode User ID***********************************************************************************/
//Moved as function setup_userid()
...
/*********Setup MQTT Client********************************************************************************/
    ...
    mqtt_client.setKeepAlive(5);
//    set_last_will_topic();
    mqtt_client.setWill(last_will_array,1,false,"offline");//topic,qos,retain,payload //Do not retail message because if App disconnect and subscribe to LWT topic then it will receive LWT though device may be connected.
//  mqtt_client.setWill("SMAYANTR_31979/SMAY_5215d1_1640ef/LWT",1,1,"offline");
    /*Create MQTT Client ID************************/ 
    //Moved as function setup_clientid()
    /*Create MQTT Client ID************************/ 
  ...
  else
  {
    ...
    if(wifi_status==WL_CONNECTED && WiFi.localIP().toString()!="0.0.0.0")
    {
        ...
        else //Device is connected to MQTT Broker
        {
          ...
          {
            if(!mqtt_topic_subscription_status[TOPIC_INT_INDEX][0]||!mqtt_topic_subscription_status[TOPIC_ON_OFF_INDEX][0] || !mqtt_topic_publish_status[PUB_TOPIC_INFO_INDEX][0] || !mqtt_topic_subscription_status[TOPIC_BEACON_INDEX][0])//if mqtt_client returns pub or sub id =0 then need to disconnect and re-connect to mqtt broker [REASON NOT KNOWN, WHY CLIENT RETURNS 0 ID, NEED TO FIND] 
            {
              Serial.println("Invalid Pub/Sub ID from Client......Disconnecting from MQTT Broker");
            ...
            {
              if(!mqtt_topic_subscription_status[TOPIC_INT_INDEX][1])//if device is connected to MQTT broker and not subscribed to "[UID]/[Device_id]/Int" topic then subscribe it; // This is for increasing robustness
              {
                Serial.println("Not subscribed to [UID]/[Device_id]/Int.....retrying to Subscribe");
                subscribe_topic(TOPIC_INT_INDEX);                
              }
              
              if(!mqtt_topic_subscription_status[TOPIC_ON_OFF_INDEX][1])//if device is connected to MQTT broker and not subscribed to "[UID]/[Device_id]/On_off" topic then subscribe it; // This is for increasing robustness
              {
                Serial.println("Not subscribed to [UID]/[Device_id]/On_off.....retrying to Subscribe");
                subscribe_topic(TOPIC_ON_OFF_INDEX);                                
              }

              if(!mqtt_topic_publish_status[TOPIC_INT_INDEX][1])
              {
                Serial.println("Publish failed for topic [UID]/[Device_id]/Info.....retrying to Publish");
                publish_topic(PUB_TOPIC_INFO_INDEX,1,true);//topic_id,qos_level,retain                                            
              }
...
void print_pub_stat(void)
{
  int i;
  Serial.printf("TOPIC\tPUB_ID\tSTATUS\n");
  for(i=0;i<TOTAL_PUBLISH_TOPIC;i++)
  {
    switch(i)
    {
      case PUB_TOPIC_INFO_INDEX:
        Serial.print("/Info");      
      break;
      case PUB_TOPIC_STATUS_INDEX:
        Serial.print("/Status");      
      break;
      default:
        Serial.print("--NA--");      
    ...
    Serial.print("\t");
    if(mqtt_topic_publish_status[i][0]!= -1)
    {
      Serial.print(" ");
      Serial.print(mqtt_topic_publish_status[i][0]);
    }
    else Serial.print(mqtt_topic_publish_status[i][0]);
    Serial.print("\t");
    if(mqtt_topic_publish_status[i][1]>0)Serial.println("TRUE");
    else Serial.println("FALSE");
  }  
}
...
void publish_topic(int topic_id,int qos_level,bool retain)
{
    ...
    switch(topic_id)
    {
      case PUB_TOPIC_INFO_INDEX:
          ...
          if(led_status)temp_str=String(clientid_array)+';'+get_device_name()+';'+'1'+';'+String(led_intensity);
          else temp_str=String(clientid_array)+';'+get_device_name()+';'+'0'+';'+String(led_intensity);
      ...
      case PUB_TOPIC_STATUS_INDEX:
          ...
          temp_str.toCharArray(mqtt_topic,100);
          temp_str="status";
          temp_str.toCharArray(temp_arr,20);
          if(led_status)temp_str=String(temp_arr)+';'+'1'+';'+String(led_intensity);
          else temp_str=String(temp_arr)+';'+'0'+';'+String(led_intensity);
          temp_str.toCharArray(mqtt_payload,100);
      break;
    }
...
//    pub_id=mqtt_client.publish(mqtt_topic,1,true,mqtt_payload);//QOS level is kept as 1 because this message should be delivered surely, App will not be able to publish any messages without this info.
    ...
    pub_id=mqtt_client.publish(mqtt_topic,qos_level,retain,mqtt_payload);                                                               
    Serial.println("Publish ID :"+String(pub_id));  
   if(qos_level>0) //because broker responds to only publish messages having qos_level greater then 0.
   {
    update_mqtt_topic_publish_id(topic_id,pub_id,false);
   }

}
/*******************************************************************************/

void clear_mqtt_topic_publish_status() //Clear All subscribtion status to false
{
 for(int i=0;i<TOTAL_PUBLISH_TOPIC;i++)
 {
  mqtt_topic_publish_status[i][0]=-1;
  mqtt_topic_publish_status[i][1]=-1;  
 }
print_pub_stat(); 
}

void update_mqtt_topic_publish_status(int sub_id,int stat)
{
  int i;
  for(i=0;i<TOTAL_PUBLISH_TOPIC;i++)
  {
    if(mqtt_topic_publish_status[i][0]==sub_id)break;
  }
  mqtt_topic_publish_status[i][1]=stat;
  print_pub_stat();
}

void update_mqtt_topic_publish_id(int sub_topic,int sub_id,int stat)
{
  mqtt_topic_publish_status[sub_topic][0]=sub_id;
  mqtt_topic_publish_status[sub_topic][1]=stat;  
//  print_pub_stat();
}
/****************************************************************************/

//void set_last_will_topic(void)
//{
//    String lwt=String(encoded_uid)+'/'+String(clientid_array)+'/'+"LWT";
//    lwt.toCharArray(last_will_array,50);
//    Serial.print("Last Will Topic :");
//    Serial.println(last_will_array);
//}
...
//  int pub_id;
  ...
  {
    publish_topic(PUB_TOPIC_INFO_INDEX,1,false);//publish device information whenever receive beacon message
  }
  else if(String(topic).equals(String(encoded_uid)+'/'+String(clientid_array)+'/'+"Int"))
  {
    ...
    publish_topic(PUB_TOPIC_STATUS_INDEX,1,false);//retain=false
  }
...
void mqtt_onconnect(bool sessionPresent)
{
    ...
    clear_mqtt_topic_publish_status();
    subscribe_topic(TOPIC_BEACON_INDEX);
/******************************************************/
    publish_topic(PUB_TOPIC_INFO_INDEX,1,false);//(topic_id,qos_level,retain)
                                               //QOS level is kept as 1 because this message should be delivered surely, App will not be able to publish any messages without this info.
                                               //initially Retain was kept true because this message should be delivered immediately after App subscribe to this "Info" Topic
                                               //Later retain message set to false as info message will be delivered to App even if device is offline.
                                                   
    publish_topic(PUB_TOPIC_STATUS_INDEX,1,false);//send current status to app retain=false
    ...
    subscribed_topic_monitor=millis();//reset monitor timer
/*******************************************************/
...
void mqtt_unsubscribe(uint16_t packetId)
{
  ...
  Serial.print("  packetId: ");
  Serial.println(packetId);
  update_mqtt_topic_subscription_status(packetId,false);
}
...
void mqtt_onpublish(uint16_t packetId)
{
  ...
  Serial.print("  packetId: ");
  Serial.println(packetId);
  update_mqtt_topic_publish_status(packetId,true);
}
...
*/
if(WiFi.status()==WL_CONNECTED && WiFi.localIP().toString()!="0.0.0.0")set_indicator(CLOUD_DISCONNECTED);  
}
/*********************************************************************************/
...
/*********Setup last will topic to intimate connected devices about abnormal disconnection****************/
void setup_last_will(void)
{
String lwt=String(encoded_uid)+'/'+String(clientid_array)+'/'+"LWT";
lwt.toCharArray(last_will_array,50);
Serial.print("Last Will Topic :");
